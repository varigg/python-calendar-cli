...
class GoogleAPIClient(ABC):
    ...
    def handle_api_error(self, error: HttpError) -> None:
        ...
        if category == "AUTH":
            raise CLIError(
                "Authentication failed. Please run 'caltool config' to re-authenticate."
            )
        elif category == "QUOTA":
            raise CLIError(
            ...
            raise CLIError(
                "Temporary connection error. Please try again in a moment."
            )
        else:  # CLIENT
            raise CLIError(
                f"API request failed. Please check your request and try again."
            )
    ...
    def retry_on_exception(
        ...
        max_retries: int = 3,
        delay: int = 2,
        allowed_exceptions: tuple = (HttpError,),
    ) -> Callable:
        """Decorator to retry a function with smart logic.
