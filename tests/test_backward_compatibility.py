...
class TestGCalClientPublicAPI:
    ...
    @patch('caltool.gcal_client.build')
    @patch('caltool.gcal_client.Credentials.from_authorized_user_file')
    def test_gcalclient_constructor_signature(self, mock_creds_from_file, mock_build, mock_config, tmp_path):
        ...
        (tmp_path / "token.json").write_text('{"token": "test"}')
    ...
    @patch('caltool.gcal_client.build')
    @patch('caltool.gcal_client.Credentials.from_authorized_user_file')
    def test_gcalclient_authentication_behavior(self, mock_creds_from_file, mock_build, mock_config, tmp_path):
        """Verify GCalClient handles authentication transparently (internal behavior)."""
    ...
    def test_gcalclient_exception_handling_preserved(self, mock_config):
        ...
        # Should raise HttpError (not swallowed or transformed)
...
class TestGCalClientRefactoringContract:
    ...
    def test_gcalclient_no_duplicate_auth_logic_after_refactor(self, mock_config):
        ...
        # This test will fail initially (before Phase 6)
        # After T028, authenticate() should be removed (handled by GoogleAuth)
        client = GCalClient(mock_config, service=Mock())
        
        # Initially this will pass (method exists), after refactor it should be removed
        # TODO: After refactor, add: assert not hasattr(client, 'authenticate')
        # For now, just document that the method currently exists
        assert hasattr(client, 'authenticate'), "authenticate() exists (will be removed in Phase 6)"
