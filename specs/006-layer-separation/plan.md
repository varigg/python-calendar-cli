...
1. Create `src/gtool/infrastructure/exceptions.py` — define infrastructure exceptions
2. Update `src/gtool/infrastructure/auth.py` — remove `click`, use infrastructure exceptions
3. Update `src/gtool/infrastructure/retry.py` — use infrastructure exceptions instead of CLI errors
4. Update `src/gtool/config/settings.py` — remove `click`, define config exceptions, move prompting logic to CLI
5. Update `src/gtool/cli/main.py` — add exception translation and interactive prompting
...
**Status**: ✅ PASS — Design preserves CLI:
...
- No circular dependencies: infrastructure → exceptions; config → exceptions; CLI → both
...
└── tasks.md             # Phase 2 output (implementation tasks) - TO BE GENERATED
```
...
├── cli/
│   ├── __init__.py
│   ├── errors.py        # CLI-specific exceptions (AuthenticationError, CLIError)
│   ├── formatters.py    # Output formatting
│   └── main.py          # Click commands ⚠️ WILL BE MODIFIED (add exception translation)
├── clients/
│   ├── __init__.py
...
├── config/
│   ├── __init__.py
│   └── settings.py      # Configuration ⚠️ WILL BE MODIFIED (remove click, add exceptions)
├── core/
│   ├── __init__.py
...
├── infrastructure/
│   ├── __init__.py
│   ├── auth.py          # Authentication ⚠️ WILL BE MODIFIED (remove click)
│   ├── error_categorizer.py
│   ├── exceptions.py    # 🆕 NEW FILE (infrastructure exceptions)
│   ├── retry.py         # Retry policy ⚠️ WILL BE MODIFIED (use infra exceptions)
│   └── service_factory.py
└── utils/
    ├── __init__.py
    └── datetime.py
...
├── __init__.py
├── conftest.py
├── test_cli.py          # ⚠️ WILL BE MODIFIED (test exception translation)
├── test_config.py       # ⚠️ WILL BE MODIFIED (expect domain exceptions)
├── test_google_auth.py  # ⚠️ WILL BE MODIFIED (expect infra exceptions)
├── test_retry_policy.py # ⚠️ WILL BE MODIFIED (expect infra exceptions)
└── [other test files]   # May need minor updates if they depend on exception types
```
...
- **NEW**: `src/gtool/infrastructure/exceptions.py` — Infrastructure exception types
...
- `ServiceError` — Google API service errors
...
- `ConfigValidationError` — Configuration validation failures (inherits from `ConfigError`)
...
- Map `AuthError` → `AuthenticationError` (CLI)
- Map `ServiceError` → `CLIError` (CLI)
- Map `ConfigValidationError` → `click.UsageError` (CLI)
...
1. Update `auth.py` — Remove `click`, use `AuthError`
2. Update `retry.py` — Use infrastructure exceptions
...
5. Add high-value integration tests (factory → auth → config flow)
...
- Factory → GoogleAuth → Config initialization flow
...
| Incomplete exception mapping     | Trace all error paths from infrastructure → CLI             |
